...
st.title("🌊 Wateran")
st.markdown("**Predict time series data with GRU - Simple, Fast, Accurate!**", unsafe_allow_html=True)
...
with col1:
    ...
    if uploaded_file:
        ...
        date_col = None
        if datetime_cols:
            date_col = st.selectbox("Select Date Column (optional)", ["None"] + datetime_cols, index=0, key="date_col_train", help="Choose a column with dates, or use index if none.")
            if date_col != "None":
        ...
        # Variable selection with form for batch updates
        with st.form(key='variable_selection_form'):
            output_var = st.selectbox("🎯 Output Variable", numeric_cols, key="output_var_train", help="Select the variable to predict.")
            ...
            if not available_input_cols:
                ...
                st.stop()
            input_vars = st.multiselect("🔧 Input Variables", available_input_cols, default=[available_input_cols[0]], key="input_vars_train", help="Select variables to use as predictors.")
            if not input_vars:
            ...
            for var in input_vars:
                var_type = st.selectbox(f"{var} Type", ["Dynamic", "Static"], key=f"{var}_type", help="Dynamic: Uses lagged values; Static: Uses current value only.")
                var_types[var] = var_type
            
            submit_vars = st.form_submit_button("Apply Variable Selection")
            if submit_vars:
...
with col2:
    ...
    with st.form(key='model_config_form'):
        ...
        num_lags = st.number_input("Number of Lags", min_value=1, max_value=10, value=DEFAULT_NUM_LAGS, step=1, key="num_lags", help="Number of past time steps to use.")
        ...
        selected_metrics = st.multiselect("Select Metrics", all_metrics, default=st.session_state.selected_metrics, key="metrics_select", help="Choose metrics to evaluate model performance.")
    ...
    if uploaded_file and st.session_state.input_vars and st.session_state.output_var:
        col_btn1, col_btn2 = st.columns(2)
        with col_btn1:
            if st.button("🚀 Train Model", help="Train the GRU model with the selected data and settings."):
                ...
                if train_size <= 0 or train_size >= len(df):
                    st.error("Invalid train split size.")
                    st.stop()
                
                train_df, test_df = df[:train_size], df[train_size:]
                if train_df.empty:
                    st.error("Training DataFrame is empty.")
                    st.stop()

                scaler = MinMaxScaler()
                data_to_scale = train_df[feature_cols + [st.session_state.output_var]].copy()
                if data_to_scale.empty or len(data_to_scale) < st.session_state.num_lags:
                    st.error("Not enough data for the selected number of lags.")
                    st.stop()
                
                if not all(pd.api.types.is_numeric_dtype(data_to_scale[col]) for col in data_to_scale.columns):
                    st.error("Non-numeric data detected in training set.")
                    st.stop()
                
                if data_to_scale.isnull().any().any():
                    st.error("NaN values detected in training data.")
                    st.stop()
                ...
                if X_train is None or y_train is None or X_train.size == 0 or y_train.size == 0:
                    st.error("Training data is invalid.")
                    st.stop()
        ...
        with col_btn2:
            if st.button("🔍 Test Model", help="Test the trained model on the holdout data."):
...
if os.path.exists(MODEL_WEIGHTS_PATH):
    with st.expander("🔮 New Predictions", expanded=False):
        ...
        new_data_file = st.file_uploader("Upload New Data (Excel)", type=["xlsx"], key="new_data", help="Upload an Excel file for prediction.")
        ...
        if st.session_state.new_data_file:
            new_df = load_data(st.session_state.new_data_file)
            ...
            if datetime_cols:
                ...
                date_col = st.selectbox("Select Date Column", datetime_cols, index=datetime_cols.index(st.session_state.new_date_col) if st.session_state.new_date_col in datetime_cols else 0, key="date_col_new", help="Choose a date column for plotting.")
            ...
            # Input variable selection with form
            with st.form(key='new_data_form'):
                ...
                selected_inputs = st.multiselect("🔧 Input Variables for Prediction", available_new_inputs, default=st.session_state.selected_inputs, key="new_input_vars", help="Select predictors for the new data.")
                ...
                if selected_inputs:
                    ...
                    for var in selected_inputs:
                        var_type = st.selectbox(f"{var} Type (New Data)", ["Dynamic", "Static"], key=f"new_{var}_type", help="Dynamic: Uses lagged values; Static: Uses current value only.")
                ...
                submit_new_data = st.form_submit_button("Predict")
                if submit_new_data:
...
# Footer
st.markdown("---")
st.markdown("**Built with ❤️ by xAI | Powered by GRU and Streamlit**", unsafe_allow_html=True)
